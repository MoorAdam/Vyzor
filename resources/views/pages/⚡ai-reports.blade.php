...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function requestAiReport(): void
    {
        ...
        if (!$projectId) {
            $this->addError('project', 'Please select a project first.');
            return;
        }
...
<div class="p-6 space-y-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <x-ui.card size="full">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <x-ui.heading level="h3" size="md">Request AI Report</x-ui.heading>
                    </div>

                    <form wire:submit="requestAiReport" class="space-y-5">
                        {{-- Preset Selection --}}
                        <x-ui.field required>
                            <x-ui.label>Report Preset</x-ui.label>
                        ...
                        {{-- Custom Prompt --}}
                        <x-ui.field>
                            <x-ui.label>Additional Instructions <span class="text-neutral-400 font-normal">(optional)</span></x-ui.label>
                            <textarea
                                wire:model="customPrompt"
                                rows="3"
                                placeholder="Add specific topics or questions you'd like the AI to address..."
                ...
                @else
                    <x-ui.card size="full" class="border-l-4 border-l-neutral-300 dark:border-l-neutral-700">
                        <div class="flex flex-col items-center justify-center py-8 text-center">
                            ...
                            <x-ui.description>Click the eye icon on a preset to preview its instructions.</x-ui.description>
                        </div>
                    </x-ui.card>
                @endif
            </div>
    ...
    @if ($activeTab === 'manual')
        <x-ui.card size="full">
            <div class="flex items-center gap-2 mb-4">
                ...
                <x-ui.heading level="h3" size="md">Write a Report</x-ui.heading>
            </div>

            <form wire:submit="createManualReport" class="space-y-5">
                <x-ui.field required>
                    <x-ui.label>Title</x-ui.label>
                    <x-ui.input wire:model="manualTitle" placeholder="Report title..." :invalid="$errors->has('manualTitle')" />
    ...
    <div>
        <div class="flex items-center justify-between mb-3">
            <x-ui.heading level="h3" size="md">Recent Reports</x-ui.heading>
            ...
            <x-ui.card>
                <x-ui.empty>
                    <x-ui.empty.contents>
                        ...
                        <x-ui.text>No reports yet. Request an AI report or write your own above.</x-ui.text>
                    </x-ui.empty.contents>
                </x-ui.empty>
            </x-ui.card>
        @else
